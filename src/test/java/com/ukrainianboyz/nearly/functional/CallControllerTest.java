...
class CallControllerTest {
    ...
    private DatabaseUser testUser1 = new DatabaseUser("id1", "kek", "", "kek@", null, 0);
    private DatabaseUser testUser2 = new DatabaseUser("id2", "lol", "", "lol@", null, 1);
    private DatabaseUser testUser3 = new DatabaseUser("id3", "gei", "", "guy@", null, 0);

    private UserRelationship validRelationship = new UserRelationship(testUser1.getUserId(), testUser2.getUserId(), testUser1.getUserName(), testUser2.getUserName(), Status.ACCEPTED);
    private UserRelationship invalidRelationship = new UserRelationship(testUser2.getUserId(), testUser3.getUserId(), testUser2.getUserName(), testUser3.getUserName(), Status.REQUEST_SENT);
    ...
    @Test
    void init_call_func() {
        FirebaseCommandRequest request = new FirebaseCommandRequest(testUser1.getUserId(), testUser2.getUserId(),"kek",false);
        HttpEntity<FirebaseCommandRequest> requestEntity = new HttpEntity<>(request);
        ...
        request  = new FirebaseCommandRequest(testUser1.getUserId(), testUser3.getUserId(),"kek",false);
        ...
        request = new FirebaseCommandRequest(testUser2.getUserId(), testUser3.getUserId(),"kek",false);
        requestEntity = new HttpEntity<>(request);
        response = restTemplate.exchange(initEndpoint, HttpMethod.POST, requestEntity, Void.class);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode(), "Status was not \"Bad request\"!");
        assertNull(response.getBody(), "Response had a body!");

        request  = new FirebaseCommandRequest("fake", testUser3.getUserId(),"kek",false);
        requestEntity = new HttpEntity<>(request);
        response = restTemplate.exchange(initEndpoint, HttpMethod.POST, requestEntity, Void.class);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode(), "Status was not \"Bad request\"!");
        assertNull(response.getBody(), "Response had a body!");

        request  = new FirebaseCommandRequest("fake", null,"kek",false);
        requestEntity = new HttpEntity<>(request);
        response = restTemplate.exchange(initEndpoint, HttpMethod.POST, requestEntity, Void.class);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode(), "Status was not \"Bad request\"!");
        assertNull(response.getBody(), "Response had a body!");
