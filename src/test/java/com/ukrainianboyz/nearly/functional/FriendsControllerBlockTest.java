...
class FriendsControllerBlockTest {
    ...
    private static final DatabaseUser testUser1 = new DatabaseUser("id1", "kek", "", "kek@", null, 0);
    private static final DatabaseUser testUser2 = new DatabaseUser("id2", "lol", "", "lol@", null, 1);
    ...
    @Test
    void first_cannot_block_if_second_blocked() {
        ...
        HttpEntity<RelationshipRequest> requestEntity = new HttpEntity<>(rr);
        ResponseEntity<Void> response = restTemplate.exchange((BLOCK_USER_ENDPOINT), HttpMethod.POST, requestEntity, Void.class);
        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode(), "User was topped blocked");
    }

    @Test
    void second_cannot_block_if_first_blocked() {
        relationshipRepository.save(new UserRelationship(testUser1.getUserId(), testUser2.getUserId(),
                "first", "second", Status.BLOCKED_BY_FIRST));
